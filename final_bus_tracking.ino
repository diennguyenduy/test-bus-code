...
void loop() {
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
    if (currentTime - lastReadTime > debounceDelay) {
      ...
      sendPostRequest(jsonData, "/api/receive-rfid/", true);
    }
    rfid.PICC_HaltA();
  }
}
...
void parseGPSInfo(const String &response) {
  ...
  if (data.length() > 0) {
    ...
    sendPostRequest(jsonData, "/api/receive-gps/", false);  // Indicate that this is not RFID data
  }
}
...
void sendPostRequest(const String &data, const char *path, bool isRFID) {
  http.beginRequest();
  http.post(path);
